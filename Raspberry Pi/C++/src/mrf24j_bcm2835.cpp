...
Mrf24j::Mrf24j(int pin_reset, int pin_chip_select, int pin_interrupt) {
    ...
    bcm2835_spi_setClockDivider(BCM2835_SPI_CLOCK_DIVIDER_1024); // 390.625kHz on RPI3, see http://www.airspayce.com/mikem/bcm2835/group__constants.html#gaf2e0ca069b8caef24602a02e8a00884e
    bcm2835_spi_chipSelect(BCM2835_SPI_CS_NONE);


    bcm2835_gpio_fsel(_pin_reset, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(_pin_cs, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(_pin_int, BCM2835_GPIO_FSEL_INPT);

}


void Mrf24j::reset(void) {
    bcm2835_gpio_write(_pin_reset, LOW);
    delay(10);  // just my gut
    bcm2835_gpio_write(_pin_reset, HIGH);
    delay(20);  // from manual
}

uint8_t Mrf24j::read_short(uint8_t address) {
    bcm2835_gpio_write(_pin_cs, LOW);
    // 0 top for short addressing, 0 bottom for read
    bcm2835_spi_transfer(address<<1 & 0b01111110);
    uint8_t ret = bcm2835_spi_transfer(0x00);
    bcm2835_gpio_write(_pin_cs, HIGH);
    return ret;
}
...
uint8_t Mrf24j::read_long(uint16_t address) {
    ...
    uint8_t ret = bcm2835_spi_transfer(0);
    bcm2835_gpio_write(_pin_cs, HIGH);
    return ret;
}
